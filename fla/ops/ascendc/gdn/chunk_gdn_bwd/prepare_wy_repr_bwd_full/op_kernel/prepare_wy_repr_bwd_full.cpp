 ...
 * \brief Prepare WY representation backward full kernel entry.
 */
...
#else
#include "prepare_wy_repr_bwd_full_struct.h"
using namespace GDN;
...
__aicore__ inline void PrepareWyReprBwdFullKernelImpl(
    ...
    GM_ADDR workspace, const PrepareWyReprBwdFullTilingData *tilingData)
{
    if ASCEND_IS_AIC {
        ::PrepareWyReprBwdFullProcess<KType, BetaType> prepareWyReprBwdFullProcess(
    ...
    if ASCEND_IS_AIV {
        AscendC::TPipe tPipe;
        ::PrepareWyReprBwdFullVectorProcess<KType, BetaType> prepareWyReprBwdFullVectorProcess(
            k, v, beta, A, dA, dw, du, g, cu_seqlens, chunk_indices, dk, dv, dbeta, dg, workspace);
...
#ifndef TORCH_MODE
extern "C" __global__ __aicore__ void prepare_wy_repr_bwd_full(
    GM_ADDR k, GM_ADDR v, GM_ADDR beta, GM_ADDR A, GM_ADDR dA, GM_ADDR dw, GM_ADDR du, GM_ADDR g,
    GM_ADDR cu_seqlens, GM_ADDR chunk_indices, GM_ADDR dk, GM_ADDR dv, GM_ADDR dbeta, GM_ADDR dg,
...
#else
    REGISTER_TILING_DEFAULT(GDN::PrepareWyReprBwdFullTilingData);
    GET_TILING_DATA_WITH_STRUCT(GDN::PrepareWyReprBwdFullTilingData, tilingData, tiling);
    if (TILING_KEY_IS(1)) {
        KERNEL_TASK_TYPE(1, KERNEL_TYPE_MIX_AIC_1_2);
        GDN::PrepareWyReprBwdFullKernelImpl<DTYPE_K, DTYPE_BETA>(
            k, v, beta, A, dA, dw, du, g, cu_seqlens, chunk_indices, dk, dv, dbeta, dg, workspace, &tilingData);
    }
#endif
}
#endif
